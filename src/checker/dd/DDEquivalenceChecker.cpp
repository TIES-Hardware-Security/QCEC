...
#include "checker/dd/applicationscheme/PositionMatchApplicationScheme.hpp"
...
EquivalenceCriterion DDEquivalenceChecker<DDType, Config>::run() {
  ...
  if constexpr (std::is_same_v<DDType, qc::MatrixDD>) {
    ...
    std::cout << "Max_nodes is " << unsigned(maxActiveNodes) << std::endl;
  }
...
void DDEquivalenceChecker<DDType, Config>::initializeApplicationScheme(
  ...
  case ApplicationSchemeType::PositionMatch:
    if constexpr (std::is_same_v<DDType, qc::MatrixDD>) {
      applicationScheme = std::make_unique<PositionMatchApplicationScheme<Config>>(
          taskManager1, taskManager2);
    } else {
      throw std::invalid_argument(
          "PositionMatch application scheme can only be used for matrices.");
    }
    break;
